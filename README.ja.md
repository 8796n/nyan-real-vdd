...
- **1バイナリで Windows 10 2004 →現行。** IddCx 1.10 ヘッダでビルドし
  実行時フロアは 1.5。新しい OS では precise present regions（1.8+）、
  フレーム経路の realtime GPU priority（1.9+）、HDR10 対応モード報告
  （1.10+・モニター毎オプトイン）が自動で有効になります。
