...
4. **完全静止のモニターはフレーム完全ゼロ（同日追試。当初の「〜1Hz
   全画面 refresh」は誤読だった）。** 定常化後の静止 VDD は ReportOnly /
   ReportAndRender / セッション既定 / `MinUpdateInterval=5s` の全条件で、
   キャプチャ開始時の初回フレーム1枚のみ・以後 20〜30 秒フレームゼロ。
   当初の「0.9fps の全画面」は **plug 直後の整定バースト**（数フレーム。
   ピクセルハッシュは全一致 = 中身が変わっていないのに全画面ダーティを
   名乗る、メタデータだけの偽陽性）を計測窓で平均した誤読。
   含意: 静止した壁のキャプチャコストは本当にゼロで、クライアント側の
   特別対応は不要。トポロジ変化直後の数フレームだけ全画面 dirty の
   偽陽性が来る（気になるならハッシュ比較でスキップ可能）。
   副産物: `DirtyRegionMode` のセッション既定は ReportOnly だった。
...
- [x] 静止画面のキャプチャ挙動（追試で完全ゼロと確定。plug 直後だけ
      ピクセル不変の全画面 dirty 偽陽性が数フレーム出る — 本文4項）
